...
/// Eventually will connect with DB
/// For now, just generates a random session and token
pub async fn create_session() -> SessionWithToken {
...
pub async fn validate_session_token(token: &str) -> SessionWithToken {
...
/// For now, just returns a dummy session
pub async fn get_session(session_id: &str) -> Option<Session> {
    ...
    Some(Session {
        ...
        secret_hash: hash_secret("dummy_secret"),
        created_at: 0,
    })
}
